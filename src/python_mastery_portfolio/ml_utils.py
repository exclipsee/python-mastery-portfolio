...
def k_fold_indices(n_samples: int, k: int = 5, random_state: int | None = None):
    rng = np.random.default_rng(random_state)
...
def cross_validate_estimator(estimator_factory: Callable[[], object], X, y, k: int = 5, scorer: Callable = None):
